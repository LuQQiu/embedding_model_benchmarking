variable "project_prefix" {
  ...
  type        = string
  default     = "lu-emb-bench-cpp" # CHANGE THIS to your unique identifier
}
...
variable "allowed_ssh_cidr" {
  description = "CIDR blocks allowed to SSH and HTTP into the instance"
  type        = list(string)
  default     = ["0.0.0.0/0"] # WARNING: Change to your IP for security! e.g., ["1.2.3.4/32"]
}

variable "instance_type" {
  description = "EC2 instance type - c7i recommended for Intel optimizations (OpenVINO)"
  type        = string
  default     = "c7i.16xlarge" # 64 vCPU, 128GB RAM, Intel Xeon 8488C Sapphire Rapids
  # Alternative: c7i.8xlarge (32 vCPU, 64GB RAM) or c7i.4xlarge (16 vCPU, 32GB RAM)
}

variable "root_volume_size" {
  description = "Root volume size in GB (needs space for models, build artifacts, dependencies)"
  type        = number
  default     = 100
}
